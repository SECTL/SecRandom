...
public sealed class CoreRuntimeServicesTests : IDisposable
{
    ...
    {
        Utils.ResetDataRootForTests();
        Utils.ConfigureDataRoot(_dataRoot);
    }
    ...
    public void ConfiguredDataRoot_UsesTheConfiguredPrivateDirectoryAndCannotChangeLater()
    {
        ...
        Assert.Throws<InvalidOperationException>(() => Utils.ConfigureDataRoot(Path.Combine(_dataRoot, "other")));
    }
    ...
    public void ProfileAndTemporaryRecordServices_PersistStableRecordIdsAtTheConfiguredRoot()
    {
        ...
        var profile = CreateProfileService(configHandler, configService);
        ...
        var temporaryRecords = new DrawTemporaryRecordService(NullLogger<DrawTemporaryRecordService>.Instance);
        ...
        var reloadedConfigHandler = new MainConfigHandler(NullLogger<MainConfigHandler>.Instance, configService);
        var reloadedProfile = CreateProfileService(reloadedConfigHandler, configService);
        var reloadedTemporaryRecords = new DrawTemporaryRecordService(NullLogger<DrawTemporaryRecordService>.Instance);
    ...
    public void DrawEngineAndFeatureAvailabilityService_WorkWithoutStaticHostResolution()
    {
        ...
        var engine = new DrawEngine(configHandler, profile, NullLogger<DrawEngine>.Instance);
        var result = engine.DrawStudent(1, _ => true);
        var featureAvailability = new FeatureAvailabilityService(configHandler);
        var changeCount = 0;
    ...
    private static FileConfigService CreateConfigService() => new(NullLogger<FileConfigService>.Instance);

    private static ProfileService CreateProfileService(MainConfigHandler configHandler, ConfigServiceBase configService)
    {
        return new ProfileService(NullLogger<ProfileService>.Instance, configHandler, configService);
    }
}

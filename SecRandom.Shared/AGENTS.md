...
## OVERVIEW
Cross-project contract layer for shared config bases, profile/list/history models, attached-settings interfaces, and utility extensions.
...
- Attached settings objects use `Guid` keys and `Dictionary<Guid, object?>`; coordinate changes with Core draw/settings consumers.
- Prefer small extension methods and plain contracts here; richer behavior belongs in `SecRandom.Core`.
- `ConfigBase` / `ProfileConfigBase` define paths and identity for persisted files; handlers and desktop storage live outside Shared.

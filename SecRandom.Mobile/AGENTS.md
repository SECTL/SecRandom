...
- `MobileShellView` is the mobile root page registered through the Core view engine. It has a labeled fixed bottom bar for `抽取`, `历史记录`, `概览`, and `设置`; `抽取` owns a top-left capsule switcher with `点名` on the left and `抽奖` on the right. The settings catalog contains exactly `通用`, `个性化`, `名单管理`, `抽取`, `公平抽取`, `备份`, `更新`, and `关于`; only settings with a mobile runtime consumer may be editable. It supports local roster/prize editing, theme selection, single-record Core draws, persistent history, temporary records, overview, and Android update checks. The backup section must explain its unavailable system-file transaction rather than expose a fake action. Do not add desktop-only settings, tray/window controls, shortcuts, OOBE, audio/notification surfaces, or fake inactive controls.

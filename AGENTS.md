...
| `SeatingChartService` | app service | `SecRandom/Services/Seating/SeatingChartService.cs` | Loads and saves the active student list's independent multi-layout seating charts. |
| `CsisInterchangeService` | app service | `SecRandom/Services/Seating/CsisInterchangeService.cs` | Validates and converts CSIS API 1 CSLS/CSPS without exposing internal record IDs. |
...
- Seating charts are persisted independently per point-call list at `data/list/seating_charts/<list>.json`, support multiple layouts, and link seats with serialized `Student.RecordId` values. Never use visible `Student.Id` or names as the internal seat identity.
...
- `main.seatingChart` is a bottom-pinned, full-width, title-hidden seat-chart page. It limits draws to valid seated students while reusing roll-call security, fairness/history, temporary records, and settings. CSIS API 1 supports standard CSLS and CSPS only; reject ESPS extensions rather than discarding their semantics.

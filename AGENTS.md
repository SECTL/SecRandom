...
- Mobile settings open the independent `SettingsView` MVE through `MobileViewHost`'s one `NavigationPage`, rather than becoming a normal bottom-bar route. Keep the mobile settings catalog as the navigation shell, but have it route to registered settings pages. The shared `MobileUpdateSettingsPage` owns `settings.update`: desktop uses `UpdateCenterService`, while mobile delegates verified APK installation to `IMobileUpdateInstaller`. Do not register or restore the deprecated mobile general, personalization, list-management, draw, backup, or about settings content pages.

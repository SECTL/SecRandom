...
- Android release signing decodes `ANDROID_SIGNING_KEYSTORE_BASE64` as (possibly URL-safe) base64: trim whitespace, translate `-`→`+` and `_`→`/`, then `Convert.FromBase64String`. Do not strip non-base64 characters blindly — deleting `-`/`_` silently corrupts the keystore, and apksigner then fails with the opaque `MSB6006: "java" exited with code 2`. The build step must validate the decoded keystore with `keytool -list` before invoking `dotnet build` so bad secrets surface with a clear message.

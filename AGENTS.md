...
- Mobile `MobileViewHost` uses Avalonia `IInputPane` to keep a focused text input above the system keyboard, including keyboard animation timing and cleanup on host detach. Android `MainActivity` must use `SoftInput.AdjustResize` so Avalonia receives keyboard occlusion updates. Windows touch input initializes `SecRandom.Platforms.Windows.WindowsTouchKeyboardIntegration` before Avalonia startup; it opens `tabtip.exe` through the system COM bridge and uses `IInputPane` to shift an occluded focused `TextBox`. Keep both platform behaviors outside Core.
...
- Mobile settings open the independent `SettingsView` MVE through `MobileViewHost`'s one `NavigationPage`, rather than becoming a normal bottom-bar route. It currently reuses the desktop settings layout and starts at the hidden mobile catalog `settings.mobile`; that catalog groups visible mobile pages from `PagesRegistryService`.

...
- First-run OOBE is a top-level window shown immediately after `BuildHost()` and before floating-window/main-app initialization. Its carousel begins with a non-blocking welcome step, then shows the collapsed-by-default custom EULA and GPLv3 acknowledgement step; `GuideCompleted` and `AcceptedEulaVersion` are both required to skip it. OOBE class/prize spreadsheet imports reuse the list-management import controls in its own right-side drawer, so their file mapping, preview, duplicate handling, and cancel behavior stay aligned with the settings list pages. EULA changes increment the app-layer OOBE version.

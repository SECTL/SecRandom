...
- Managed draw music is app-layer only: `settings.personalized.music` imports/deletes/previews MP3/WAV/FLAC files in `data/audio/music`, while the four draw-settings pages select process/result tracks through managed IDs, no-music, or random-play options. Deleting a managed track clears all default and override references. `DrawAudioService` keeps SoundFlow private, loops process music only when `MoreSettings.BackgroundMusicLoop` is enabled, stops it on cancellation, and plays result music once.

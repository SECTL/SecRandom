...
- Mobile settings open the independent `SettingsView` MVE through `MobileViewHost`'s one `NavigationPage`, rather than becoming a normal bottom-bar route. Keep the mobile settings catalog as the navigation shell, but have it route to desktop settings pages; only `MobileUpdateSettingsPage` remains mobile-specific because it delegates verified APK installation to `IMobileUpdateInstaller`. Do not register or restore the deprecated mobile general, personalization, list-management, draw, backup, or about settings content pages.

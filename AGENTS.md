...
| Mobile startup | `SecRandom/App.axaml.cs`, `SecRandom.Mobile.Shared/`, `SecRandom.Android/`, `SecRandom.iOS/` | The shared `App` owns the one Host and branches by Avalonia lifetime; the mobile library provides the SingleView root, routes, and platform seams, while heads own entry points. |

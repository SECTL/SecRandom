...
| Backup settings UI | `General/BackupSettingsPage.axaml(.cs)` | The page is one `TabStrip` + `Carousel` pair. The 本地备份 tab lists real backup ZIPs under app data and delegates create/restore to `IImportExportService`, which validates archives and creates the required pre-restore snapshot. The desktop-only 云端备份 tab owns account state, cloud quota (auto-fetched on page entry, after sign-in, and after an automatic backup, with its own refresh command and 正在获取 loading label), an 打开 思拓创联 云储存页面 row that opens `https://sectl.cn/dashboard/cloud` through `IExternalLauncher.TryOpenUri`, 立即备份到云端 with a transient 上传中ing~ status plus cancel instead of a part-progress card, its own 云端备份内容 rows (`CloudInclude*`; no log, proof, or theme row because logs, the device identity, the voice cache, draw proofs, and theme resources can never be uploaded), a 设备别名 row (`BackupConfig.CloudDeviceAlias`, persisted through the page's existing `Settings.PropertyChanged` save boundary and shown in the cloud file names; empty means the host name), and 云端自动备份 rows (switch/interval/limit persisted as `BackupConfig.CloudAutoBackup*`, independent of the local auto backup; the limit is per device, while an exhausted quota still deletes the account-wide oldest backup), and cloud backup management (download-restore/delete/cleanup) behind a separator with a left-aligned 刷新中ing~/清理中ing~ status label; an upload refreshes the cloud list whether it succeeds or fails. The cloud tab item carries the `S_CloudBackup` search anchor. The page resolves `SectlAuthService`/`CloudBackupService` with `TryGetService`, hides the entire cloud tab when either is missing (mobile), delegates all cloud work to `CloudBackupService`, and must not cache cloud state in configuration. |
| Security settings | `General/SecuritySettingsPage.axaml(.cs)` | Page ID `settings.general.security`; belongs to the General navigation group. Its 防篡改 section holds the settings-file integrity switch, the 篡改处理方式 picker (`SettingsIntegrityAction`: password confirmation or automatic restore), and the restore-source picker (`SettingsIntegrityRestoreSource`, shown only while automatic restore is selected); both pickers are plain `SettingOption<T>` ComboBoxes persisted through the security-gated update boundary. |
...
- The voice page registers OmniTTS (engine 2) on every non-iOS platform (`!OperatingSystem.IsIOS()` stays). Its OmniTTS-only rows (provider, API base URL, API key, model, voice, instructions, MiMo design description/clone reference, batch cache, clear cache) live in one section bound to `IsOmniTtsSelected` and appear only while `VoiceEngine == 2`; model and voice rows are editable ComboBoxes so the user can always type values, with a fetch button pulling the model list from the provider API. MiMo clone imports require explicit authorization and WAV input; the reference is private under `data/config/voice/` and is never serialized into settings. API keys never enter the settings model — they go through `OmniTtsCredentialStore` (`data/config/voice/omnitts-keys.json`).

...
| Draw settings | `Picking/DefaultDrawSettingsPage.axaml(.cs)`, `Picking/RollCallDrawSettingsPage.axaml(.cs)`, `Picking/LotteryDrawSettingsPage.axaml(.cs)`, `Picking/FaceDetectorSettingsPage.axaml(.cs)` | Default and roll-call draw settings show only fields consumed by the current roll-call runtime; lottery keeps core draw settings used by the draw engine/plugin path. QuickDraw settings files remain on disk but are not registered until a runtime entry exists. |

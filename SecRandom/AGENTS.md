...
| Add main page                | `Views/MainPages/`, `BuildHost()`                                       | `[PageInfo]` + `AddMainPage<T>()`; built-in main navigation currently includes roll-call, lottery, history, and face draw. `main.faceDraw` is bottom-pinned directly after lottery with `VideoPersonSparkleFilled`; it auto-starts preview, shows a live count in recognize mode, and provides a timed `- / count / +` face-pick flow with cropped result display in pick mode. It has no identity recognition or student matching. Quick draw opens from the floating window. |
...
- Face draw main page ID is `main.faceDraw`; it is bottom-pinned, full-width, title-hidden, uses `VideoPersonSparkleFilled`, follows lottery in the sidebar, and is registered unconditionally as `人脸抽取`.
- `main.faceDraw` follows the V2 interaction: start preview on load, show live count in recognize mode, and show only a count stepper plus timed start/stop picking control in pick mode. Device, resolution, detector, and duration settings remain on the face-draw settings page. The main page renders selected faces as cropped final results, then restores the live preview.
...
- `DebugSettingsPage` intentionally skips its localization folder and remains guarded by `#if DEBUG`. `CameraPreviewTestPage` is the unconditionally registered `main.faceDraw` page and uses its navigation title `人脸抽取`.
